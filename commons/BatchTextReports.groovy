		...
		private void requestParams(){
			...
			if (batch080Details != null){
				...
				edoi.search(query,{MSF081Rec msf081Rec ->
					...
					Integer repeatCounter = Integer.parseInt(msf081Rec.getRepeatCnt())
					...
					while(repeatCounter-- > 0){
						...
						if (paramEndIndex > 0 && Integer.parseInt(msf081Rec.getOffsetMsf080())-1 < paramEndIndex && endIndex080 <= maxLength  ){
							paramValue = batch080Details.getRequestParams().substring(startIndex080,endIndex080)
						}else {
							paramValue = SPACES
						}
						...
						if (isParamInList){
							requestParams080.put(msf081Rec.getDescLine_1().trim() + " " + paramIndex.toString(),paramValue)
							paramIndex++
						}else{
